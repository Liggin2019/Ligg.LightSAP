...
namespace Ligg.Uwa.Mvc.Controllers
{
    public class FileController : BaseController
    {
        ...
        [HttpPost]
        public async Task<TResult<string>> Upload(string index, string obj, string objId, string uploadType, string newFileTitle, IFormCollection fileList, string avCode)
        {
            uploadType = uploadType.IsNullOrEmpty() ? "upload" : "update";
            if (fileList == null) return new TResult<string> { Flag = 0, Message = "Upload failed" };
            IFormFile uploadedFile = fileList.Files[0];
            var uploadedFileName = uploadedFile.FileName;
            var fileNameToBeSave = newFileTitle.IsNullOrEmpty()? uploadedFileName : newFileTitle + Path.GetExtension(uploadedFileName);
            var absolutepath = await GetAbsolutePathAsync(index, obj, objId, fileNameToBeSave, uploadType.ToLower()=="update"? UploadType.Update: UploadType.Upload, uploadedFileName);
            if (absolutepath == null) return new TResult<string> { Flag = 0, Message = "Incorrect url" };
            if (absolutepath == string.Empty) return new TResult<string> { Flag = 0, Message = "You have no authorization" };

            var allowedSuffixes = "";//getFrConfig
            var rst = await WebFileHelper.Upload(absolutepath, allowedSuffixes, fileList.Files, uploadType == "update");
            return rst;
        }
