...
(function ($, ys) {
    ...
    $.extend(ys, {
        ...
        importData: function (url, postData) {
            ys.ajax({
                url: url,
                type: "post",
                data: postData,
        ...
        checkTextLenth: function (elementId, elementName, min = 0, max = 1023) {
            var elmt = document.getElementById(elementId);
            if (!elmt) { ys.alertError(elementId + " 不存在"); return false; }
            var ctrl = $(elmt);
            var len = ctrl.val().length;
            if (len < min) { ys.alertError("'" + elementName + "'" + "长度:" + len+", 不能小于" + min); return false; }
            if (len > max) { ys.alertError("'" + elementName + "'" + "长度:" + len +", 不能大于" + max); return false; }
            return true;
        },
        checkNumeralValue: function (elementId, elementName, min = 0, max = 999999) {
            var elmt = document.getElementById(elementId);
            if (!elmt) { ys.alertError(elementId + " 不存在"); return false; }
            var ctrl = $(elmt);
            var val = ctrl.val();
            if (val < min) { ys.alertError("'" + elementName + "'"  + "不能小于" + min); return false; }
            if (val > max) { ys.alertError("'" + elementName + "'" + "不能大于" + max); return false; }
            return true;
        },

        checkSelectedValue: function (elementId, elementName, value) {
            var elmt = document.getElementById(elementId);
            if (!elmt) { ys.alertError(elementId + " 不存在"); return false; }
            var ctrl = $(elmt);
            var val = ctrl.val();
            if (val == value) { ys.alertError("请选择"+elementName); return false; }
            return true;
        },

        checkComboboxSelection: function (elementId, elementName) {
            var elmt = document.getElementById(elementId);
            if (!elmt) { ys.alertError(elementId + " 不存在"); return false; }
            var ctrl = $(elmt);
            var val = ctrl.val(); 
            if (val == -1) { ys.alertError("请选择" + elementName); return false; }
            //var index = ctrl.get(0).selectedIndex;
            //if (index == 0) { ys.alertError("请选择" + elementName); return false; }
            return true;
        },
    });
